...
from pyspark.sql.functions import col, sum, cast, row_number, first, countDistinct, size, collect_set
...
# group per game-id into a row with winners and losers
# rename and move
# write to csv
games =  spark.read.csv("tichu_data/2games.csv", header = True)\
    ...
    .groupBy("game-id", 'draw').pivot("row_number", [1, 2, 3, 4]).agg(first("player"))\
    .select(col("game-id"), col("1").alias("winner1"), col("2").alias("winner2"), col("3").alias("loser1"),col("4").alias("loser2"), col('draw'))\
    .write.csv("games.csv", header=True, mode="overwrite")
